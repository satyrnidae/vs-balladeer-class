...
using VSBalladeerClass.Instruments;
...
public abstract class BalladeerModCommon : ModSystem
{
    ...
    public const string CLAUSTROPHOBIC_COMMENT =
        "If enabled, the balladeer will be significantly worse at mining. (-10% speed, -15% ore drop rate)";

    public const string OVERCONFIDENT_COMMENT =
        "If enabled, the balladeer will have somewhat worse ranged and melee damage. (-15% each)";

    public const string BRASH_COMMENT =
        "If enabled, the balladeer will be louder by default and have a somewhat increased animal detection range. (+15%)";

    public const string FRAIL_COMMENT =
        "If enabled, the balladeer will have less health and significantly less bow draw strength. (-2.5 hp, -25% bow draw str)";
    ...
    private void LoadConfiguration(ICoreAPI api)
    {
        ...
        api.World.Config.SetBool("balladeerEnableClaustrophobic", _configuration.Traits.Claustrophobic);
    ...
    private void ValidateConfig()
    {
        ...
        Configuration.Traits.__Claustrophobic_Comment = CLAUSTROPHOBIC_COMMENT;
        Configuration.Traits.__Overconfident_Comment = OVERCONFIDENT_COMMENT;
        Configuration.Traits.__Brash_Comment = BRASH_COMMENT;
        Configuration.Traits.__Frail_Comment = FRAIL_COMMENT;
    }
}
